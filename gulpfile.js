...
gulp.task('default',function()
{
	...
	function buildPackage(i){
		var pt=[]
		if(i.pageType.length>0){
			pt= i.pageType.split('_')
		}else{
			pt=["",""]
		}
		var data={
		...
		if(!isBuild(i)){
			...
			gulp.src(['./tmp/package/'+data.type+'.html','./tmp/package/'+data.type+'.css','./tmp/package/'+data.type+'.js'])
				.pipe(handlebars(data))
				.pipe(rename({basename:data.en}))
				.pipe(gulp.dest(data.url))

			return console.log('成功构建'+data.type+'类别模块:'+ i.en)
	...
	function buildObj(i){
		...
		if(!isBuild(i)){
			...
			gulp.src(['./tmp/obj/obj.js'])
				...
				.pipe(gulp.dest("./obj/"+ i.ModuleName+'/'))
