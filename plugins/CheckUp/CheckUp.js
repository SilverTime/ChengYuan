/**
 * Created by mooshroom on 2016/5/7.
 *
 * 调用CheckUp()方法检查字段类型
 * ---------------------------
 * 传入：
    ...
    return CheckUp = function (val, type, inputID) {
        ...
        var checkUtil = {
            pwd: function () {     //匹配密码
                ...
                //内容匹配
                if (reg.match(/[a-zA-Z]+/g)===null) {
                    return [false, "密码必须含有英文字母"];
                }
                if (reg.match(/\d+/g)===null) {
            ...
            username: function () {
                //长度匹配
                if (val.length>20) {
                    return [false, '密码错误：长度应不大于20'];
                }
                var reg = /[a-zA-Z]\w{0,20}/;        //长度在0~20之间，只能包含字母、数字和下划线。
                if (reg.test(val)) {
                    return [true, ''];
                }
